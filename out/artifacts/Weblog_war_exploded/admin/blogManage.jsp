	...
	<script type="text/javascript">
		...
		function deleteBlog(){
			...
			$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/blogAndTypeServlet?way=deleteBlog",{ids:ids},function(result){
						if(result){
		...
		function openBlogModifyTab(){
			...
			window.parent.openTab('修改博客','blogAndTypeServlet?way=getUpdateBlogByIda&id='+row.id,'icon-writeblog');
		}
...
<table id="dg" title="博客管理" class="easyui-datagrid"
	   ...
	   url="/admin/blogAndTypeServlet?way=getBlogAndTypeList" fit="true" toolbar="#tb">
	<thead>
	<tr>
	...
	<c:forEach items="${blogAndTypeList}" var="blog">
		<tr>
			<td><%--type="checkbox"--%></td>
			<td>${blog.id}</td>
			<td><a href="/blogServlet?way=getBlogById&id=${blog.id}">${blog.title}</a></td>
