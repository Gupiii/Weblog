...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(content==null || content==''){
			...
			$.post("${pageContext.request.contextPath}/admin/commentServlet?way=addComment",{'content':content,/*'imageCode':imageCode,*/'blog.id':'${blog.id}'},function(result){
				if(result.success){
					// window.location.reload();
					<%response.sendRedirect("foreground/blog/view.jsp");%>
...
<div class="data_list">
	<div class="data_list_title">
		...
		评论信息    
		<c:if test="${commentByBlogId.size()>10}">
			<a href="javascript:showOtherComment()" style="float: right;padding-right: 40px;">显示所有评论</a>
		</c:if>
	</div>
	<div class="commentDatas">
		<c:choose>
			<c:when test="${commentByBlogId.size()==0}">
				暂无评论
			</c:when>
			<c:otherwise>
				<c:forEach var="comment" items="${commentByBlogId }" varStatus="status">
						<c:choose>
