...
<script type="text/javascript">
	...
	function deleteBlogType(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/blogTypeServlet?way=deleteBlogType",{ids:ids},function(result){
						if(result){
							// if(result.exist){
							...
							// }
							$.messager.alert("系统提示","数据已成功删除！");
							$('#dg').datagrid("options").url = '${pageContext.request.contextPath}/admin/blogTypeManage.jsp';
							 // $("#dg").datagrid("reload");
                            location.reload();
						}else{
	...
	function openBlogTypeAddDialog(){
		...
		url="${pageContext.request.contextPath}/admin/blogTypeServlet?way=addBlogType";
	}
	...
	function openBlogTypeModifyDialog(){
		var selectedRows=$("#dg").datagrid("getSelections");
		 ...
		 $("#fm").form("load",row);
		 url="${pageContext.request.contextPath}/admin/blogTypeServlet?way=updateBlogType&id="+row.id;
	 }
...
<table id="dg" title="博客类别管理" class="easyui-datagrid"
   ...
   url="${pageContext.request.contextPath}/admin/blogTypeManage.jsp" fit="true" toolbar="#tb">
   <thead>
   <input type="hidden" value="${blogType.id}" id="id">
   	<tr>
