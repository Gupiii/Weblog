...
<script type="text/javascript">
	...
	function commentReview(state){
		...
		$.messager.confirm("系统提示","您确定要审核这<font color=red>"+selectedRows.length+"</font>条评论吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/commentServlet?way=updateCommentState",{ids:ids,state:state},function(result){
					if(result.success){
	...
	function formatBlogTitle(val,row){
		if(val==""){
			...
			return "<a target='_blank' href=''>"+val+"</a>";
		}
	}
...
<table id="dg" title="评论审核管理" class="easyui-datagrid"
   ...
   <%--url="/admin/commentServlet?way=getCommentState&state=0"--%> fit="true" toolbar="#tb">
   <thead>
   	<tr>
	...
	<c:forEach items="${commentState}" var="comList">
		<tr>
			<td></td>
			<td>${comList.id}</td>
			<td>${comList.blog.id}:${comList.blog.title}</td>
			<td>${comList.userIp}</td>
			<td>${comList.content}</td>
			<td>${comList.commentDate}</td>
		</tr>
	</c:forEach>
 </table>
 <div id="tb">
