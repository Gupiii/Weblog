...
<script type="text/javascript">
	...
	function deleteLink(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/linkServlet?way=deleteLink",{ids:ids},function(result){
						if(result.success){
							 $.messager.alert("系统提示","数据已成功删除！");
							location.reload();
							 // $("#dg").datagrid("reload");
							<%--location.reload("${pageContext.request.contextPath}/admin/linkServlet?way=getLinkList")--%>
						}else{
	...
	function openLinkAddDialog(){
		...
		url="${pageContext.request.contextPath}/admin/linkServlet?way=addlink";
	}
	...
	function openLinkModifyDialog(){
		var selectedRows=$("#dg").datagrid("getSelections");
		 ...
		 $("#fm").form("load",row);
		 url="${pageContext.request.contextPath}/admin/linkServlet?way=updateLinkById&id="+row.id;
	 }
...
<table id="dg" title="友情链接管理" class="easyui-datagrid"
   ...
   <thead>
	...
	<c:forEach items="${linkListAdmin}" var="link">
		<tr>
			<td></td>
			<td>${link.id}</td>
			<td>${link.linkName}</td>
