	...
	function deleteComment(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/commentServlet?way=deleteComment",{ids:ids},function(result){
						if(result){
...
<table id="dg" title="评论管理" class="easyui-datagrid"
   ...
   url="<%--${pageContext.request.contextPath}--%>/admin/commentServlet?way=getCommentList" fit="true" toolbar="#tb">
   <thead>
   	<tr>
	...
	<c:forEach items="${commentList}" var="comList">
		<tr>
			<td></td>
			<td>${comList.id}</td>
			<td>${comList.blog.title}</td>
